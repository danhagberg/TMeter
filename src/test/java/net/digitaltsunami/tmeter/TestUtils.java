...
public class TestUtils {
     ...
     *         stripLineFeed.
     */
    public static String getTimerLogOutput(Timer timer, boolean stripLineFeed) {
        // Set timer output to byte array so the value can be captured.
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        Timer.setOut(new PrintStream(out));
        timer.start();
        timer.stop();
        String outputLine = out.toString();
        Timer.setOut(System.out);
     ...
     */
    public static String getTimerLogOutput(Timer timer) {
        return getTimerLogOutput(timer, false);
    }
